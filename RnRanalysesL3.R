...
for(dd in 1:m){
  ...
  for(cn in unique(hold$country)){
    ...
    hold.cn[4:nrow(hold), 5:ncol(hold.cn)] = hold.cn[1:(nrow(hold)-3), 5:ncol(hold.cn)]
    hold.cn[1:3,] = NA
...
for(k in 1:3){
  ...
  parLapply(cl, 1:m, function(i){
    ...
    rm(fit)
    write.csv(get(paste0('fitSum.', k, '.', i)), file = paste0('~/Dropbox/JudicialAnalysis/JudicialIndependence/fitSumL3.', k, '.', i, '.csv'))
